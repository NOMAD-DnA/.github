...
추가 push는 기존 PR에 자동 반영되므로 리뷰 수정마다 새 PR을 만들 필요는 없습니다. 승인·필요한 검사·충돌 해결 상태를 확인한 뒤 합의된 담당자가 병합합니다.
...
Squash 병합 후에는 변경이 반영되었어도 이력이 달라 `branch -d`가 거절될 수 있습니다. **PR 병합 여부와 해당 브랜치에 추가 미반영 커밋이 없는지 확인**한 경우에만 아래처럼 로컬 브랜치를 삭제합니다. 확신이 없으면 남겨 두세요.

```bash
git branch -D docs/update-workflow
```

`-D`는 강제 삭제이므로 확인 없이 사용하지 않습니다. 다음 작업은 갱신된 main에서 새 브랜치를 만듭니다.
