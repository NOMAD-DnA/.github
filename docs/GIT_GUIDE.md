...
작성자 외 **저장소 쓰기 권한이 있는 팀원**이 리뷰합니다.

1. PR 설명에서 변경 목적·관련 이슈·테스트 결과·미검증 항목을 읽습니다.
2. **Files changed**에서 변경을 확인하고, 필요한 줄의 **+** 버튼으로 의견을 남깁니다. 여러 의견을 모아 제출하려면 **Start a review**를 사용합니다.
3. **Review changes**를 누르고 아래 결과 중 하나를 선택합니다.
   - **Approve:** 변경을 확인했고 병합에 동의합니다.
   - **Request changes:** 병합 전에 반드시 수정할 내용이 있습니다. 이유와 수정 기준을 적습니다.
   - **Comment:** 질문이나 제안만 남깁니다. **필수 승인 수에 포함되지 않습니다.**
4. **Submit review**를 눌러 제출합니다. 작성 중인 리뷰나 일반 댓글만으로는 승인이 완료되지 않습니다.
5. 변경 커밋으로 승인이 해제되면 바뀐 내용을 다시 확인하고 **Approve → Submit review**로 재승인합니다. Request changes를 남겼던 리뷰어도 수정 확인 후 다시 승인합니다.

리뷰 대화는 수정·답변이 실제로 해결되었는지 확인한 뒤 **Resolve conversation**으로 정리합니다. 대화 해결과 PR 승인은 별개이며, 승인만으로 CI 실패나 미해결 대화를 건너뛸 수 없습니다.

### 리뷰 후 일반 merge로 병합하기

1. 작성자 외 팀원의 승인, 필요한 CI 통과, 리뷰 대화 해결, 충돌 없음 상태를 확인합니다.
2. GitHub PR에서 **Create a merge commit** 방식을 선택합니다.
3. **Merge pull request**를 누르고 병합 메시지를 확인합니다. **병합 커밋 제목은 GitHub 기본값인 `Merge pull request #번호 …`를 유지**한 뒤 **Confirm merge**합니다. `type: 변경 내용` 형식은 일반 작업 커밋과 PR 제목에 사용합니다.
...
작업 브랜치는 **자동 삭제하지 않고 작성자가 수동 정리**합니다.

1. GitHub에서 PR이 **Merged** 상태인지 확인합니다.
2. 원격·로컬 작업 브랜치에 추가 미반영 커밋이나 진행 중인 작업이 없는지 확인합니다. 공동 작업 중이면 다른 작업자에게도 확인합니다.
3. 확인이 끝나면 GitHub PR 화면의 **Delete branch**로 원격 작업 브랜치를 삭제합니다. 확신이 없으면 삭제하지 않습니다.
4. 로컬 변경을 먼저 커밋하거나 임시 보관한 뒤, 아래 명령으로 main을 갱신하고 로컬 작업 브랜치를 정리합니다. 브랜치명은 본인 작업 이름으로 바꿉니다.
...
`fetch --prune`은 이미 삭제된 원격 브랜치의 로컬 추적 정보만 정리하며, 로컬 작업 브랜치 자체를 삭제하지 않습니다.
