...
- main 강제 push와 삭제를 제한합니다. **작업 브랜치 자동 삭제는 사용하지 않습니다.** 작성자가 병합 완료와 추가 미반영 작업이 없음을 확인한 후 원격·로컬 작업 브랜치를 수동 정리합니다. 공동 작업자의 미완료 작업이 있거나 확신이 없으면 남겨 둡니다.
...
일반 작업 커밋과 PR 제목은 `type: 변경 내용` 형식을 쓰고, 병합 커밋 제목은 GitHub 기본 `Merge pull request #번호 …`를 유지합니다. 병합 커밋 제목을 작업 커밋 형식으로 바꾸지 않습니다.
